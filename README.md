...
Created .env.development file to use GraphQL Playground instead of GraphiQL  
```shell
...
**npm run develop**  
```shell
env-cmd -f .env.development gatsby develop -H 0.0.0.0  
```
This loads the ENV variables  
Added -H 0.0.0.0 to develop command in package.json to use the host's IP  
...
JS library for parsing markdown files  
...
I found gatsby-plugin-sharp would not install on my Raspberry Pi, worked fine on Ubuntu  
